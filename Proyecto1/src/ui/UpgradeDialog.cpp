...
UpgradeDialog::UpgradeDialog(
  int slotIndex, const Economy& economy, QWidget* parent) : QDialog(parent) {
  setWindowTitle(
    QStringLiteral("Ranura %1 — elegir estructura").arg(slotIndex));

  auto* layout = new QVBoxLayout(this);

  auto* creditsLabel = new QLabel(
    QStringLiteral("Créditos disponibles: %1").
    arg(economy.getCredits()), this);
  ...
  for (CoreType type : allCoreTypes()) {
    ...
    auto* item = new QListWidgetItem(
      QStringLiteral("%1 — %2 créditos").
      arg(coreDisplayName(type)).arg(price));
